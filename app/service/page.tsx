  ...
  const sendVerificationCode = async () => {
    try {
      const response = await fetch('/api/verify-phone', {
        ...
        body: JSON.stringify({ phoneNumber: `+1${formData.phoneNumber}`}),
      })
  ...
  const verifyCode = async () => {
    ...
    try {
      const response = await fetch('/api/check-verification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phoneNumber: `+1${formData.phoneNumber}`, code: verificationCode }),
      })
      const data = await response.json()
    ...
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>Book your bike service and verify your phone number</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-2">
              ...
              <div className="flex space-x-2">
                ...
                <Button type="button" onClick={sendVerificationCode} disabled={isVerified}>
                  {isVerified ? 'Verified' : (isCodeSent ? 'Resend Code' : 'Send Code')}
                </Button>
              </div>
            </div>
            {isCodeSent && !isVerified && (
              <div className="space-y-2">
                ...
                <div className="flex space-x-2">
                  ...
                  />
                  <Button type="button" onClick={verifyCode} disabled={isVerifying}>
